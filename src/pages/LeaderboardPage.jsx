...
function LeaderboardPage() {
  ...
  const currentUser = prepared.find((entry) => entry.id === user.id)
  ...
  return (
    <main className="leaderboard-page">
      ...
      <aside className="sticky-rank" aria-label="Your sticky rank bar">
        <span>You are rank <strong>#{currentUser?.rank ?? 'N/A'}</strong></span>
        <span className={currentUser?.weeklyChange > 0 ? 'delta-up' : currentUser?.weeklyChange < 0 ? 'delta-down' : 'delta-flat'}>
          Weekly: {currentUser?.weeklyChange > 0 ? `↑ ${currentUser.weeklyChange}` : currentUser?.weeklyChange < 0 ? `↓ ${Math.abs(currentUser.weeklyChange)}` : '—'}
        </span>
      </aside>
    </main>
  )
